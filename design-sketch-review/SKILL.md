...
| 终点 | 评审通过 → 输出正式 md 给执行 agent，并由 md 生成定稿版 HTML | 一次性 HTML 阅读版，不建档 |
...
- 草图路线图必须分解到 milestone → slice → task 三级（编号 M<n> / M<n>.S<m> / T<k>）：评审就在这个粒度上拍板，定稿后 md 的"任务分解"直接沿用这套结构，可交给 execute-plan 执行。
...
2. **生成正式 md**（面向执行 agent，规范见下文"正式 md 写作规范"），统一输出到项目 `docs/design/` 目录（不存在则创建），文件名 `<日期>_<主题>.md`，用户指定路径时从其指定。
3. **一致性自检**：逐项核对草图中的每张决策卡、每条红线、每个 milestone / slice / task 在 md 里都有对应落实，发现遗漏立即修正 md。
4. **收尾**：history.json 的 `phase` 改为 `"finalized"`、`final_doc` 记 md 路径；追加一轮定稿记录（feedback 记 `"评审通过，定稿"`，status 记 `"已定稿"`）；md 快照存 `rounds/final-rNN.md`。
5. **定稿版主 HTML 从正式 md 全新生成**：以 md 为事实源走一遍可视化流程——页面结构对齐 md 章节（目标与范围 / 总体设计 / 详细设计 / 任务分解 / 约束红线 / 验收标准 / 决策记录 / 风险与开放问题），内容取 md 的精确表述（保留术语与表格），仅呈现形式用可视化组件。在此基础上注入两块评审资产：history.json 的完整迭代记录面板（含定稿轮卡片）与 emerald 色 Hero 徽章 `已定稿 · 正式文档见 <md 文件名>`；草图的交互控件与导出栏不带入。**禁止在迭代草图的 HTML 上打补丁充当定稿版**——草图是大白话评审形态，定稿页面必须与 md 一一同源，否则 md 与页面两份"真相"开始漂移。

**定稿后再有修改意见**：事实源已切换为正式 md——按迭代协议六步执行（改 md → history 留痕 → 从 md 重新生成定稿版主 HTML → 存档轮次副本 → 汇报），`phase` 保持 `"finalized"`、轮次继续累加，md 与定稿 HTML 永远同源生成；修改量大或用户要求重新走评审时，再回到阶段 1 重开草图。
...
## 4. 任务分解
   （milestone → slice → task 三级分解，写法见下文"任务分解写法"，
   与 execute-plan skill 的计划格式约定对齐，定稿后可直接作为执行计划）
## 5. 约束与红线
## 6. 验收标准
...
### 任务分解写法（与 execute-plan 对齐）

第 4 节内部固定用三级结构，编号全程连续、不复用：
...
- [ ] T1 做什么（§3.2）
- [ ] T2 做什么（§3.3）
- [ ] 验收：<可判定的完成标准>（本 slice 的 DoD）

里程碑门禁：
- [ ] <milestone 级整体验收项>（可选）
```

- **粒度标准**：milestone = 可独立验证、可独立交付的一段成果；slice = 一个可独立测试的增量（对应 execute-plan 的一次提交单元）；task = 单个工作单元。
- **依赖必须显式声明**：milestone 正文写依赖与目标；slice 首行写依赖（无依赖也要写"无"）。
- **每个 slice 末尾必须有"验收"task** 作为完成定义（DoD）；milestone 末尾可放"里程碑门禁"小节。
- **task 用 §x.x 引用本 md 的详细设计章节**——任务描述只是索引，约束细节都在正文，执行者必须回读原章节。
- 标题层级说明：execute-plan 约定独立计划文件里 milestone 用 `## M<n>`、slice 用 `### M<n>.S<m>`；嵌在本 md 第 4 节下时整体降一级（`###` / `####`），编号与复选框约定完全一致，execute-plan 可按语义直接解析。
- **交接执行**：定稿汇报时告知用户——把本 md 路径作为执行计划交给 execute-plan 即可开始按 milestone 开发。
...
- 模式 A 定稿后（含再迭代）：事实源 = 已定稿的正式 md——改 md，再从 md 重新生成定稿版主 HTML（见阶段 3 第 5 步）
...
├── 2026-08-19_概念数据接入.html      ← 永远是最新版（评审期 = 草图；定稿后 = 由正式 md 生成的定稿版）
...
1. **写入路径**：当前项目文件夹下的 `.doc-visualizer-output/`：
   ...
   ```
2. **文件名**：主 HTML 为 `<日期>_<主题简称>.html`；存档目录同名（去 `.html`）。正式 md 统一输出到项目 `docs/design/`（不存在则创建），文件名 `<日期>_<主题>.md`。
3. **打开文件**（WSL/Linux 优先 xdg-open，失败提示路径）：
   ...
   ```
4. 告知用户：主 HTML 绝对路径、当前轮次与阶段（评审中/已定稿）、本轮改动板块、待定决策数量（若有）；定稿轮还需告知正式 md 路径，并提示"可将该 md 作为执行计划交给 execute-plan 开始开发"。
...
- [ ] 定稿版主 HTML 以正式 md 为事实源**全新生成**（页面结构对齐 md 章节、内容取 md 表述），不是在草图 HTML 上修补；迭代记录面板与已定稿徽章照常注入
- [ ] md 第 4 节任务分解为 milestone → slice → task 三级：M/S/T 编号连续、依赖显式声明（无依赖写"无"）、每个 slice 末尾有"验收"task、task 用 §x.x 引用详细设计章节
- [ ] 定稿时：无遗留"等你拍板"决策（未拍板的按默认建议落实并注明）；md 通过一致性自检（每张决策卡/红线/每个 milestone/slice/task 都有落实）；history.json 的 `phase`/`final_doc` 已更新；md 快照已存入 rounds/
...
- `可以了，输出正式文档吧`（→ 阶段 3：定稿，生成 md 给执行 agent；之后可用 execute-plan 直接执行该 md）
